...
import { COLORS_LIGHT, COLORS_DARK, COLORS_HC } from '@/components/ui/donutColors'
...
function DonutLegend({ data, colors, activeIndex, getRowIcon, onEnter, onLeave }: LegendProps) {
  return (
      ...
      {data.map((entry, i) => {
        ...
        return (
          <div key={entry.name} role="listitem">
            <button
              type="button"
              className={`w-full flex items-center gap-1 px-1.5 py-0.5 rounded text-xs font-medium transition-colors cursor-default text-left min-w-0 ${activeIndex === i ? 'bg-muted/60' : 'hover:bg-muted/40'}`}
...
export default function DonutChart({ title, tooltipLabel, tab, onTabChange, userQuery, globalQuery, defaultMode = 'user', showUserToggle = true, getRowIcon, className = '' }: Props) {
  ...
  return (
    <div className={`flex flex-col gap-2 rounded-xl border border-border bg-card p-2 select-none [&_svg]:outline-none [&_svg_*]:outline-none${className ? ` ${className}` : ''}`}>
      <div className="flex items-center justify-between gap-3">
        ...
        {showUserToggle && (
          <ToggleSwitch
            options={[{ value: 'user', label: t('dashboard.mode.user') }, { value: 'global', label: t('dashboard.mode.global') }]}
            value={mode}
            onChange={setMode}
          />
        )}
        ...
        <div key={mode} className="flex flex-1 items-stretch gap-2 min-h-0 animate-fade-in" role="figure" aria-label={title}>
          <div ref={containerRef} className="flex-1 min-h-0 flex items-center justify-center overflow-hidden" aria-hidden="true">
            <div className="relative" style={{ width: chartPx, height: chartPx }}>
              {chartPx > 0 && (
                <ResponsiveContainer width={chartPx} height={chartPx}>
                  <PieChart tabIndex={-1}>
                    <Pie
                      rootTabIndex={-1}
                      data={data.map((d, i) => ({ ...d, fill: colors[i % colors.length], fillOpacity: activeIndex === null || activeIndex === i ? 1 : 0.2 }))}
              ...
              {active && (
                <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                  <span className="text-xl font-bold text-foreground tabular-nums leading-tight">{activePct}%</span>
                  <span className="text-xs font-medium text-foreground text-center leading-tight px-4 truncate max-w-full">{active.name}</span>
                  <span className="text-xs text-muted-foreground text-center leading-tight capitalize">{tooltipLabel}</span>
                </div>
              )}
            </div>
          </div>
          <DonutLegend data={data} colors={colors} activeIndex={activeIndex} getRowIcon={getRowIcon} onEnter={setActiveIndex} onLeave={() => setActiveIndex(null)} />
        </div>
      )}
    </div>
  )
