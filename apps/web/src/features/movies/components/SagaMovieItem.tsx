...
export default function SagaMovieItem({ part, index, isCurrent, isWatched, onSelect, language }: Props) {
  const { t } = useTranslation()
  const [showTrailer, setShowTrailer] = useState(false)
  const trailerRef = useRef<HTMLDivElement>(null)

  const { trailer } = useTrailer(['movie-trailer', part.id], () => fetchMovieVideos(part.id), true, language)
  ...
  return (
    <div className={clsx(
      ...
      >
        <span className="text-xs font-mono w-4 text-center text-muted-foreground shrink-0">{index + 1}</span>
        ...
        <div className="flex flex-col min-w-0 flex-1">
          <Text variant="small" className="truncate text-foreground">{part.title}</Text>
          {part.release_date && (
            <Text variant="caption" className="text-muted-foreground">{new Date(part.release_date).getFullYear()}</Text>
          )}
        </div>
