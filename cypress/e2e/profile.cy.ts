...
describe('Profile modal — admin', () => {
  ...
  it('shows WRONG_PASSWORD error inline on current password field', () => {
    ...
    cy.get('[data-cy="profile-button"]').click()
    cy.get('[id="pw-current"]').click().type('WrongPass1!').should('have.value', 'WrongPass1!')
    cy.get('[id="pw-new"]').click().type('NewPass1!').should('have.value', 'NewPass1!')
    cy.get('[id="pw-confirm"]').click().type('NewPass1!').should('have.value', 'NewPass1!')
    cy.contains('button', 'Save password').click()
    cy.wait('@wrongPw')
  ...
  it('shows success banner on successful password change', () => {
    ...
    cy.get('[data-cy="profile-button"]').click()
    cy.get('[id="pw-current"]').click().type('OldPass1!').should('have.value', 'OldPass1!')
    cy.get('[id="pw-new"]').click().type('NewPass1!').should('have.value', 'NewPass1!')
    cy.get('[id="pw-confirm"]').click().type('NewPass1!').should('have.value', 'NewPass1!')
    cy.contains('button', 'Save password').click()
    cy.wait('@changePw')
