...
| Home dashboard — monthly recommendations (top movies & series by genre, unwatched only) | ✓ | ✓ |
...
The sidebar displays your avatar and username at the bottom. Clicking it opens the profile panel.
...
| `profile.cy.ts` | 16 | **Admin** (2): profile button not visible in sidebar, no username greeting. **Guest** (14): pencil toggle (`[data-cy="avatar-edit-btn"]` aria-pressed); pencil collapses + resets; color groups (skin tone, hair color, hair style, shirt color, glasses, expression); save avatar (PATCH /api/profile + body.avatar); error alert on failed save; password fields visible (pw-current/pw-new/pw-confirm); WRONG_PASSWORD inline error; success banner + fields cleared after successful change; axe check (collapsed + avatar editor expanded) |
