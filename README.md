...
![Version](https://img.shields.io/badge/version-0.22.0-6B2737)
...
| `profile.cy.ts` | 23 | **Admin**: profile button visible + aria-label; modal open/close; avatar edit button hidden; password section landmark; type=password inputs; eye toggle; empty form validation; WRONG_PASSWORD inline; success/error banners; fields cleared on success; username greeting in sidebar; axe check (modal open). **Guest**: pencil toggle (`[data-cy="avatar-edit-btn"]` aria-pressed); color groups (skin tone, hair color, hair style, shirt color, glasses, expression); save avatar (PATCH /api/profile + body.avatar); error alert on failed save; axe check (collapsed + avatar editor expanded) |
