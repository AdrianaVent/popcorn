...
| `profile.cy.ts` | Profile button visible + aria-label; open modal, close via ×; pencil button aria-pressed=false, click expands editing (aria-pressed=true + Save avatar visible), click again collapses; color groups labeled (role=group+aria-label: skin tone, hair color, hair style, shirt color, glasses); save avatar: PATCH /api/profile + body.avatar, editing closes on success; error alert on failed save; password section landmark; inputs type=password; eye toggle (current field); validation errors on empty submit; WRONG_PASSWORD inline error; success banner (Password updated); server error banner; fields cleared on success; Hi greeting visible in sidebar |
