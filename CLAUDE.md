...
│   │   └── stats/              # GET (admin only) — { total, guests, admins, thisMonth, byMonth[] }
...
│   │                           # CastCard (actor/director card — renders as <button> with group-hover border+ring when onClick provided; translateCharacter applied to sub text; HC ring uses ring-primary),
│   │                           # CastSection (horizontal scroll of cast cards + director row + creators row; onPersonClick?: (id) => void; py-1 on scroll container prevents ring clipping)
...
│                               # Ribbon (diagonal poster ribbon — size sm (table) / md (card/modal); colorClass prop; aria-hidden; data-cy forwarded),
...
│                               # PasswordFields (generic 3-field presentational component — current/new/confirm; values/errors/show/onChange/onToggleShow props; used in PasswordChangeSection and UserFormModal isSelf),
│                               # PasswordChangeSection (standalone password-change form with own state, validation, flash banners, save button; calls /api/profile PATCH; used in ProfileModal),
...
│   ├── seasonal.ts             # SEASONAL_CONFIG — 12-month genre map (movieGenres, seriesGenres, seriesExcludeGenres?); SeasonalMonth type
...
│   ├── home/                   # HomeFeature — genre donut charts + Top10 cards + release calendar + stats card + drag-reorder
│   │   ├── components/         # ReleaseCalendar (month grid, dots on release days, navigation),
│   │   │                       # StatsCard (thin shell — role-aware title + Activity/Summary ToggleSwitch in header for guest; delegates to stats/ subfolder),
│   │   │                       # stats/GuestStats (Activity tab: 5 chips + ChartFooter; Summary tab: InsightsTab),
│   │   │                       # stats/AdminStats (4 chips + ChartFooter; fetches /api/users/stats via useQuery),
│   │   │                       # stats/InsightsTab (avg rating chip + ratings histogram; by-decade bar chart; empty state; hasRatings/hasContent guard; data-cy="stats-no-insights"),
│   │   │                       # stats/ChartFooter (PeriodToggle — day/week/month ToggleSwitch + area/bar chart),
│   │   │                       # stats/StatChip (value + label tile; optional suffix),
│   │   │                       # stats/StatsCharts (MiniAreaChart, MiniBarChart — Recharts wrappers),
│   │   │                       # stats/statsCard.types.ts (ChartEntry, Period, StatTab, UserStats, DAY_MS, WEEK_MS),
│   │   │                       # stats/statsCard.utils.ts (buildRatingHistogram, buildDecadeDistribution, getLast6MonthBuckets, getLast6Buckets),
...
│   │   │                       # DraggableCard (dnd-kit sortable wrapper; drag handle overlay with theme-aware grip button; disabled: !isDragMode),
│   │   │                       # SeasonalPanel (two-column movie+series recommendations; SeasonalColumn + ItemRow; MONTH_ICONS lucide map; role="region")
...
│   │                           # useSeasonalRecommendations (5-page discover fetch for movies + series; seriesExcludeGenres support; 24h staleTime)
│   ├── movies/
...
│   │   │                       # useMovieInTheaters, useMovieCredits (cast + crew via /movie/{id}/credits; 24h staleTime),
...
│   ├── person/
│   │   ├── PersonModal.tsx     # filmography modal — ProfilePhoto, CreditRow, tab switcher (movies/series); mergeCredits deduplicates cast+crew (cast priority), sorted by date desc; getYear extracts 4-digit year; hasPage filter: id > 0 && vote_count > 0; onCreditClick triggers cross-type navigation (movie ↔ series) then closes PersonModal; calcAge(birthday, referenceDate) exported; header shows full birthday + age in parens + deathday range if present + place_of_birth; also_known_as filtered (person.name excluded, max 2); credits list scrollable (max-h-[50vh]); CreditRow uses translateJob for crew (character === undefined) and translateCharacter for cast
│   │   └── hooks/              # usePersonDetail (queryKey: ['person-detail', id, language], 24h staleTime, enabled: id !== null),
│   │                           # usePersonCredits (queryKey: ['person-credits', id, language], 24h staleTime; returns { cast, crew, loading })
...
│   ├── homeStore.ts            # per-user home card order (cardOrders: Record<userId, CardId[]>); persisted as 'popcorn-home-v1'
...
├── types/                      # tmdb.ts (TMDBPerson includes deathday: string | null, also_known_as: string[]), movie.ts (MovieFilters includes in_theaters?: boolean), series.ts (genre_ids explicit to avoid unknown from index sig),
...
└── utils/
    ...
    ├── translateCharacter.ts   # translateCharacter — word-boundary regex for Self/Himself/Herself/Themselves/Voice/Narrator/Guest/(uncredited)/Archive footage; defaultValue: match fallback; translateJob — exact-match (^…$) for 16 common TMDB crew job titles (Director, Producer, Executive Producer, Screenplay, Writer, DOP, Composer, Editor, Casting, Production Designer, Costume Designer, VFX Supervisor, Stunt Coordinator, Actor's Assistant, Co-Producer, Story); both exported
...
| Home stats card — role-aware: guest sees activity chips (movies/series/episodes/avg rating) + monthly activity bar chart; admin sees user stats chips (total/guests/admins/this month) + monthly registrations bar chart; `/api/users/stats` endpoint with ms-range bucket queries | Done |
| Home drag-and-drop card reordering — "Organizar" toggle button in header; drag mode shows dashed border + dimmed overlay + grip handle (theme-aware: burgundy/yellow/red); order saved per user on exit; `homeStore` persists `cardOrders: Record<userId, CardId[]>` | Done |
| Profile avatar customization — mouth/expression picker (6 options via DiceBear `mouth[]`); compact 3-row layout (hair style + expression / glasses + glasses color / skin + hair color + shirt); pencil button inverted style when active; `parseAvatar` validates `mouth` and `glassesColor` against allowed sets for backward compat; `GLASSES_COLORS` reduced to 6; smile/smirk label swap (DiceBear inversion) | Done |
| Monthly recommendations (seasonal panel) — star (★) button in release calendar header (guest only — hidden for admin); two-column panel (Movies + Series) filtered by month theme; `SEASONAL_CONFIG` maps 12 months to TMDB genre IDs with optional `seriesExcludeGenres` (Feb: excludes Sci-Fi/Mystery/Action/Crime/War; Apr: excludes War/Action/Crime/Documentary); `MONTH_ICONS` maps months to lucide icons; fetches 5 pages (100 results pool) per column; watched titles excluded via `watchedStore`; sorted by `vote_average` desc; top 10 per column; `SeasonalPanel` → `SeasonalColumn` → `ItemRow` component hierarchy; `role="region"` + `aria-busy` + `aria-label` per item button; `useSeasonalRecommendations` hook with 24h staleTime | Done |
| Sidebar profile area — guest only (avatar + "Hola, username" greeting); admin sees only the logout button at the bottom; `PasswordFields` (generic 3-field presentational component) + `PasswordChangeSection` (standalone form with flash banners + save button) extracted as shared components; `PasswordChangeSection` used in `ProfileModal` (guest); `PasswordFields` used in `UserFormModal` (edit self — 3-field current→new→confirm flow integrated into Aceptar; no pw field when editing other users; role selector uses `appearance-none` + `ChevronDownIcon`); seasonal recommendations button in release calendar hidden for admin (guest only) | Done |
| Actor/director filmography — `PersonModal` opened from CastSection (actors, director, creators); `mergeCredits` deduplicates cast+crew (cast priority), sorted by date desc; `hasPage` filter removes vote_count=0 entries; Movies/Series tabs with ARIA tablist/tabpanel; watched badge per credit; cross-type navigation (clicking TV credit in MovieDetailModal opens SeriesDetailModal via `next/dynamic`, and vice versa); `usePersonDetail` + `usePersonCredits` (24h staleTime); `translateCharacter` utility for Self/Voice/Narrator etc. | Done |
| PersonModal biography info — header shows full birthday (formatShortDate + age in parens); deathday range if present (age at death); place_of_birth; also_known_as filtered (person.name excluded, max 2, italic); credits list scrollable (max-h-[50vh]); CreditRow applies `translateJob` for crew and `translateCharacter` for cast; `translateCharacter` extended with himself/herself/themselves; `translateJob` covers 16 common TMDB crew jobs | Done |
| "En cartelera" filter for movies — boolean filter in FiltersPanel; when true, `fetchMovies` calls `/movie/now_playing?region=ES` instead of discover; `FilterFieldInput` boolean case gains `data-cy`; `isFieldActive` fixed to treat boolean `false` as inactive (bug: `false` was previously counted as an active filter) | Done |
| Generic `Ribbon` component — replaces inline ribbon divs in `MediaTableCells` (sm, "Visto"), `SeriesDetailModal` (md, status color), `SeriesCard` (md, "En progreso"); size sm/md controls position + width; `colorClass` prop; `aria-hidden`; `data-cy` forwarded | Done |
...
| Area | What's covered |
|---|---|
| Pure functions | `getMovieUI`, `getSeriesUI`, `updateFilterValue`, `getTMDBImageUrl`, `resolveMode`, `formatVoteCount`, `formatShortDate`, `tmdbToStarRating` (TMDB 0–10 → Rating 0.5–5), `deduplicateProviders` (generic, subtype preservation), `buildGenreCounts` (aggregate, sort, slice top-10; per-entry name dedup), `applyRuntimeFilter` (undefined/0 threshold → passthrough, empty runtimes map → passthrough, filters by total duration, exact threshold kept, null rt passes through, missing from map passes through), `binPackSagas` (empty input, single row, two groups fitting/not fitting, gap-filling before new row, displayedCounts used for width), `computeSagasFirst` (sagas-first when saga watchedAt > standalone; standalone-first when no sagas; tie-break by most-recent watchedAt), `computeAllMovies` (SagaCard helper — rating-filtered watched movies still show as watched via full store; genuinely unwatched returns null; regression test for old group.movies approach), `parseJSON` (valid array, non-array throws, malformed throws), `parseCSV` (3-col, header-only, empty input, commas in password ×2, 5-col, 5-col with commas, trims whitespace), `mergeCredits` (empty arrays, cast priority over crew for same id, sorted by date desc, no-date sorts last, empty role fallback to job), `getYear` (release_date, first_air_date, both null → '—', slices to 4 chars), `translateCharacter` (Self/Himself/Herself/Themselves/Voice/Narrator/Guest/(uncredited)/Archive footage; word-boundary regex; compound roles; defaultValue fallback; 11 cases), `translateJob` (16 crew job titles, exact ^…$ match, case-insensitive; unknown job passthrough; Art Director not matched by Director; 12 cases), `calcAge` (age before/after/on birthday; age at death via deathday; zero case; 5 cases), `buildRatingHistogram` (empty inputs, movie ratings, movie+series combined, all-zero entries), `buildDecadeDistribution` (empty, movies by decade, series by decade, mixed, trims empty leading buckets, year range labels), `getLast6MonthBuckets` (6 buckets, chronological order, contiguous, YYYY-MM key format, key matches start date, one calendar month each), `getLast6Buckets` weekly (6 buckets, span = WEEK_MS, contiguous, last ends at tomorrow midnight), `getLast6Buckets` daily (6 buckets, span = DAY_MS, contiguous, daily < weekly) |
| Business logic | `applyClientFilters` (movies + series + language filter), `tmdbFetch` error mapping, `toCSV` (headers, quoting, empty rows) |
| Store | `watchedStore` — `toggleMovie`, `toggleEpisode` (seasonNumber, watchedAt), `markSeason` (watchedAt), per-season count derivation, auto-remove from watchlist on mark (movie/episode/season); `watchlistStore` — `toggleMovie` (add, addedAt, remove, per-user isolation), `toggleSeries` (add, addedAt, remove, per-user isolation), `removeMovie`/`removeSeries` (no-op safety); `toastStore` — addToast, timers, removeToast; `ratingsStore` — setRating, removeRating, per-user isolation; `homeStore` — empty initial state, DEFAULT_CARD_ORDER fallback, setCardOrder per user, per-user isolation, overwrite existing order |
| Services | `fetchMovies — in_theaters` (calls nowPlaying when true; calls discover when false/absent; passes correct language; 4 cases) |
| Hooks | `useMovieDetail`, `useSeriesDetail` (conditional fetch via `enabled`), `useWatchProviders` (flatrate/rent/buy merge, dedup, source tagging, loading), `useMovieInTheaters` (type 3 release, 90-day window), `useMovieReleases` (service call args), `useSeriesReleases` (disabled when no providers, enabled with providers), `useUserMovieTop10` / `useUserSeriesTop10` (genre_ids backfill via detail fetch, staleTime 5min), `useTrailer` (language preference via iso_639_1, YouTube filter, allTrailers list; disabled/enabled), `useEnrichedTrailers` (YouTube oEmbed batch fetch, replaces TMDB names with real YouTube titles, 24h cache), `useMovieRuntimeEnrichment` (empty map, populates runtime, null when no runtime, null on fetch fail, multiple movies, mixed success/failure), `useFilters` (initial state, setFilters, replaces entire object, preserves initial reference, exposes function), `usePersonDetail` (null id → no fetch; loading state; success; error → 'TMDB_FETCH_ERROR'), `usePersonCredits` (null id → no fetch; loading state; cast/crew returned; empty arrays on error) — all wrapped in `QueryClientProvider` with `retry: false` |
| Components | `PersonModal render` (birthday renders, deathday renders, age suffix, alias shown, alias matching name excluded, max 2 aliases, cast Himself → translateCharacter, crew Executive Producer → translateJob, unknown crew job passthrough, Director cast not affected by translateJob; 11 cases) |
| Components | `Button`, `Modal`, `FiltersPanel` (collapse/expand, badge count, text/number/star/boolean/date/select types), `SeriesMetaGrid`, `ExportButton`, `ConfirmModal`, `UserFormModal`, `ToastItem`, `WatchProviders` (loading skeleton, badges, inTheaters chip), `ErrorBoundary` (children render, fallback on error, retry reset), `MediaPoster` (image render, null fallback, error fallback, loading prop, fluid variant, error recovery on URL change), `ReleaseCalendar` (header, Today button visibility, day selection, releases panel, X close, onEntryClick, no-overview state, loading/error states; axe: 7 states; ARIA: region, prev/next/close labels, full-date aria-label on day buttons, aria-pressed on selected), `CalendarReleaseItem` (heart visible for guest, hidden for admin, hidden when already watched; watchlist toggle — aria-pressed false/true, calls toggleMovie with release data; genre icon deduplication — one icon when two genre_ids share the same icon, two icons when genres differ), `StarRating` (5 stars, readonly mode, onChange, hover, half-star gradient), `GenreGrid` (renders badges; deduplicates by resolved name + icon — Avatar case: Action+Adventure share icon → one badge), `TrailerPlayer` (iframe with correct src, no close button without onClose, close button present and calls onClose, custom className), `SearchableSelect` (trigger label, X clear, open/close, search filtering, case-insensitive, empty state, option highlight, outside-click close), `YearRangePicker` (fromOptions capped at valueTo, toOptions floored at valueFrom, null callbacks), `FilterFieldInput` (all 8 field types: text/number/boolean/select/date/star/searchable-select/year-range/genre-multi; null guard for missing options/keyTo), `ToggleSwitch` (renders both labels, active has bg-primary, inactive does not, onChange with value, onChange on active option, role="group", reflects rerender), `MediaTableCells` — TitleCell (renders title, tooltip disabled when not truncated), GenresCell (null for undefined/empty, renders icons, multiple genres, deduplication by icon, filters null icons), PosterCell (renders poster, ribbon present when isWatched, ribbon absent when not watched, translated label), `StatusBadge` (undefined → animate-pulse, Ended, unknown status, Returning Series, In Production), `MediaCard` (axe, aria-label, onClick, children, selected ring), `MovieCard` (axe, title, year, StarRating, Similar button), `SeriesCard` (axe, name, episode badge, StarRating readonly state, Similar button), `WatchlistCard` (axe, title, year, remove button), `RecommendationsDrawer` (axe, complementary role, source name, items, excludes watchedIds, excludes saga siblings, onClose, onSelect) |
| Services | `apiFetch` (401 auto-refresh, redirect on session expiry) |
| API routes | `/api/users/import` (per-row validation: missing fields, invalid role/password, intra-file duplicate, DB duplicate, invalid creator, invalid date); `/api/users/stats` (returns error from requireAdmin, returns stats when admin, passes req, 401 response) |
| Components (cont.) | `Ribbon` (renders label, aria-hidden, colorClass applied, sm size classes, md default, data-cy forwarded); `StatChip` (renders value, renders label, string value, suffix present/absent); `DraggableCard` (renders children, no handle when !isDragMode, handle visible when isDragMode, children still render in drag mode, works for all CardId values); `StatsCard` — guest: title, 5 chips in Activity tab, empty state, movie count, avg dash/star in Summary tab, activity chart visibility, period toggle aria-pressed (weekly default), period toggle update, tab toggle visible (Activity/Summary), tab switches to Summary, no insights empty state; admin: title, 4 chips, chart, registrations chart, period toggle aria-pressed (monthly default), period toggle update, loading skeleton, error state; `ReminderItem` (role="button" + tabIndex=0, click → onEntryClick, Enter key → onEntryClick, Space key → onEntryClick, other keys ignored; heart shown for guest, hidden for admin, hidden when watched; aria-pressed="false"/"true" on watchlist toggle; click heart calls handleToggleWatchlist; genre icon deduplication — one icon when two genre_ids share the same icon, two icons when genres differ); `AvatarDisplay` (correct src, null opts → DEFAULT_AVATAR, size applied as px, initial letter fallback on error, aria-hidden wrapper); `ColorSwatchGrid` (button per color, aria-label from t(labelKey), aria-pressed on selected, checkmark on selected, onSelect callback, background color style, cols prop for gridTemplateColumns); `ProfileModal` (axe: collapsed + avatar editing expanded; avatar section guest-only (hidden for admin); avatar preview, pencil aria-pressed/expanded, clicking pencil shows options, color groups role=group+aria-label incl. mouth, closing pencil hides options, save avatar success/error, password inputs type=password, eye toggle, validation errors on empty form, WRONG_PASSWORD inline error, success/error banners with role=alert, fields clear on success, pw-hint aria-describedby); `PasswordFields` (renders 3 inputs; all type=password by default; type=text when show[field]=true; onChange called with field+value; onToggleShow called with field; aria-label changes to hidePassword when visible; error messages per field); `UserFormModal` isSelf (3-field pw section shown; single pw field hidden; pw fields absent when editing other user; validation error when partial fields filled; mismatch error; /api/profile PATCH called before onSubmit; WRONG_PASSWORD inline without closing modal; skips /api/profile when pw fields empty) |
| Config | `avatars.ts` — `parseAvatar` (null/undefined/empty → DEFAULT_AVATAR, malformed JSON → DEFAULT_AVATAR, partial merge, legacy glasses: true/"true"/"false"/false/null, mouth fallback for unknown/missing, glassesColor fallback for removed colors), `serializeAvatar` round-trip (incl. mouth), `buildAvatarUrl` (seed, hair/colors, mouth, glassesProbability 0/100, glasses style+color, special chars encoded) |
...
| `movies.cy.ts` | Movie list, detail modal, watch providers, platform filter, star rating filter, genre multi-select filter, genre deduplication in modal, access control (guest), watched controls (admin/guest), trailer (button visible, open iframe, close via button, close via × in player), column sort (rating asc/desc — verifies sort_by param sent to TMDB), runtime filter (2h → 120min, 90min — verifies with_runtime.gte param), watchlist heart button (guest: visible + gains active style on click; admin: hidden); En cartelera filter (checkbox calls /movie/now_playing, results render); person modal (actor opens PersonModal, known_for_department shown, Movies/Series tabs visible, vote_count=0 credits filtered, clicking movie credit navigates to that movie, clicking TV credit opens SeriesDetailModal, director opens PersonModal, birthday year visible, also_known_as alias visible, Himself translated in Series tab, axe with PersonModal open) |
| `series.cy.ts` | Series list, detail modal, watch providers, platform filter, star rating filter, genre multi-select filter, genre deduplication in modal, watched controls (admin/guest), trailer (button visible, open iframe, close via button, close via × in player), column sort (rating asc/desc — verifies sort_by param sent to TMDB), runtime filter client-side (filters out series below total duration threshold; verifies with_runtime.gte NOT sent to TMDB), watchlist heart button (guest: visible + gains active style on click; admin: hidden); person modal (actor opens PersonModal, known_for_department shown, clicking TV credit navigates to that series — uses within() to switch to Series tab, clicking movie credit opens MovieDetailModal, creator opens PersonModal, axe with PersonModal open) |
...
| `home.cy.ts` | Home header, content tab switch (Movies/Series), toggle defaults to Global when no watched data, My profile/Global toggle, empty state message, genre chart SVG renders, release calendar title and navigation, Top10 card title + year visible, calendar trailer button, Top10 genre dropdown (open, close, genre-filtered fetch, label update), calendar watchlist button (admin: hidden; guest: visible; toggle aria-pressed on/off; hidden when movie already watched); StatsCard admin (Users title, 4 chips, registrations chart, period toggle default/update, loading skeleton, axe); StatsCard guest (My activity title, 5 chip labels, empty state, axe; activity chart + period toggle with localStorage-seeded data, axe); StatsCard guest Resumen tab (Activity/Summary buttons visible, Activity default aria-pressed, empty state via data-cy="stats-no-insights", avg rating + chart with seeded ratings, axe with Summary open); Reminders panel (button hidden for admin, visible for guest, opens panel with Today + Upcoming sections, closes via X button, empty state, watchlisted movie in Today section, watchlisted movie in Upcoming section, axe); Seasonal panel (button visible for guest only — hidden for admin; aria-label/aria-pressed, opens panel replacing star button with CloseBar, Movies/Series column headings, movie titles from discover API, series names from discover API, X closes panel, clicking movie opens detail modal, axe); Drag mode toggle (Organizar button visible + aria-pressed=false, activates with 4 handles, exits on second click, visible for admin, axe in drag mode) |
| `my-list.cy.ts` | Page header + tabs, empty state (movies/series), nav item hidden for admin / visible for guest, watched movie visible, Recommendations button disabled without rating / enabled after ≥ 3.5★, saga name formatted (no "Collection"), section ordering (standalone-first / saga-first by watchedAt), series tab + episode progress badge + "Finish to rate", "In progress" ribbon visible on in-progress series / absent on completed series, recommendations drawer (opens, header title, close via X, close via Escape, items from TMDB, already-watched items excluded, clicking item opens detail modal), series recommendations items, saga drawer header name, unwatched saga placeholders visible, clicking placeholder opens detail modal, Por ver tab (visible, empty state, movie/series visible, count badge, remove on heart click), single-released-movie collection shown as standalone (not saga group); saga tests stub /collection API so CI placeholder TMDB key does not demote 2-part sagas |
| `profile.cy.ts` | **Admin** (2 tests): profile button not visible, no username greeting. **Guest** (14 tests): pencil aria-pressed toggle (`[data-cy="avatar-edit-btn"]`), pencil collapses + resets, color groups role=group+aria-label (skin tone/hair color/hair style/shirt color/glasses/expression), save avatar (PATCH /api/profile + body.avatar), error alert on failed save, password fields visible (pw-current/pw-new/pw-confirm), WRONG_PASSWORD inline error, success banner + fields clear after successful change, axe check collapsed + expanded |
| `users.cy.ts` | (password change self-edit): 3-field password section present when editing own account, single password field absent, WRONG_PASSWORD inline, success banner on successful change |
