...
│   ├── person/
│   │   ├── PersonModal.tsx     # filmography modal — ProfilePhoto, CreditRow, tab switcher (movies/series); mergeCredits deduplicates cast+crew (cast priority), sorted by date desc; getYear extracts 4-digit year; hasPage filter: id > 0 && vote_count > 0; onCreditClick triggers cross-type navigation (movie ↔ series) then closes PersonModal
...
└── utils/
    ...
    ├── translateCharacter.ts   # word-boundary regex for Self/Voice/Narrator/Guest/(uncredited)/Archive footage; defaultValue: match fallback
