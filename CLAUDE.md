...
| `profile.cy.ts` | **Admin** (12 tests): profile button visible + aria-label, modal open/close, avatar edit btn hidden (guest-only guard), password section landmark, type=password inputs, eye toggle, empty form validation, WRONG_PASSWORD inline, success/error banners, fields cleared, sidebar greeting, axe check. **Guest** (11 tests): pencil aria-pressed toggle (`[data-cy="avatar-edit-btn"]`), pencil collapses + resets, color groups role=group+aria-label (skin tone/hair color/hair style/shirt color/glasses/expression), save avatar (PATCH /api/profile + body.avatar), error alert on failed save, axe check collapsed + expanded |
